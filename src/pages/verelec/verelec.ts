import { Component } from '@angular/core';
import { IonicPage, NavController, NavParams } from 'ionic-angular';
import { FavoritosProvider } from '../../providers/favoritos/favoritos';

/**
 * Generated class for the VerelecPage page.
 *
 * See https://ionicframework.com/docs/components/#navigation for more info on
 * Ionic pages and navigation.
 */

@IonicPage()
@Component({
  selector: 'page-verelec',
  templateUrl: 'verelec.html',
})
export class VerelecPage {
  dispositivo = [];
  imgs = [];
    constructor(public navCtrl: NavController, public navParams: NavParams, public fav: FavoritosProvider) {
      this.dispositivo = this.navParams.get('dispositivo');
  
      if(this.dispositivo.ad.hasOwnProperty('images')){
        this.imgs = this.dispositivo.ad.images;
      }
    }
  
    ionViewDidLoad() {
      console.log('ionViewDidLoad VisualizacionPage');
    }
    
    favoritos(dispositivos){
      this.fav.addfavoritos(dispositivos);
    }
  }